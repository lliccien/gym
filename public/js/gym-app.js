document.addEventListener('DOMContentLoaded', () => {
    ...
    function switchView(viewToShow) {
        ...
        if (viewToShow === 'history') {
            ...
            tabHistory.classList.add('border-blue-500', 'text-blue-500', 'font-semibold');
            
            tabToday.classList.remove('border-blue-500', 'text-blue-500', 'font-semibold');
            tabToday.classList.add('border-transparent', 'text-gray-500');
        } else {
            tabToday.classList.remove('border-transparent', 'text-gray-500');
            tabToday.classList.add('border-blue-500', 'text-blue-500', 'font-semibold');
            
            tabHistory.classList.remove('border-blue-500', 'text-blue-500', 'font-semibold');
    ...
    // --- VERIFICAR EJERCICIOS DE HOY ---
    function checkTodayExercises() {
        if (!db) return;
        ...
        request.onsuccess = (e) => {
            ...
            if (cursor) {
                if (cursor.value.routineId == currentRoutineId) {
                    completedExercises.add(cursor.value.exercise);
                }
                cursor.continue();
            } else {
                // Marcar ejercicios como completados en la UI
                completedExercises.forEach(exerciseName => {
                    const registerBtn = document.querySelector(`.register-btn[data-exercise="${exerciseName}"]`);
                    if (registerBtn) {
                        const card = document.getElementById(registerBtn.dataset.cardId);
                        if (card) {
                            card.classList.add('bg-green-50');
                            registerBtn.textContent = '✓ Registrado';
                            registerBtn.disabled = true;
                            registerBtn.classList.replace('bg-blue-500', 'bg-green-600');
                            
                            // Buscar el registro para mostrar las calorías
                            findWorkoutData(exerciseName, (workoutData) => {
                                if (workoutData) {
                                    const existingCalorieDisplay = registerBtn.nextElementSibling;
                                    if (!existingCalorieDisplay || !existingCalorieDisplay.textContent.includes('Kcal')) {
                                        ...
                                        calorieDisplay.textContent = `🔥 ${workoutData.calories} Kcal (aprox.)`;
                                        registerBtn.insertAdjacentElement('afterend', calorieDisplay);
                                    }
                                }
                            });
                        }
        ...
        request.onerror = (e) => {
            console.error('Error al buscar registros de ejercicios:', e.target.error);
        };
    }

    function findWorkoutData(exerciseName, callback) {
        if (!db) return;
        ...
        request.onsuccess = (e) => {
            const cursor = e.target.result;
            if (cursor) {
                if (cursor.value.exercise === exerciseName && cursor.value.routineId == currentRoutineId) {
        ...
        request.onerror = (e) => {
            console.error('Error al buscar datos del ejercicio:', e.target.error);
            callback(null);
        };
    }
